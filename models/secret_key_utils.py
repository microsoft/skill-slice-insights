...
def GetKey(key_name, local_keys_path=None, key_vault_url=None):
    ...
    if key_value is None:
        if key_vault_url is None:
            raise ValueError("Key not found in local keys file and key_vault_url is not provided.")
        else:
            key_value = get_key_from_azure(key_name, key_vault_url)

    # if the key still wasn't found, raise an error
    if key_value is None:
        raise ValueError("Failed to get key from both local keys file or Azure key vault.")
...
def get_key_from_azure(key_name, key_vault_url):
    ...
    try:
        logging.info("Trying to get the key from Azure Key Vault using DefaultAzureCredential")
        credential = DefaultAzureCredential(additionally_allowed_tenants=["*"])
        client = SecretClient(vault_url=key_vault_url, credential=credential)
        retrieved_key = client.get_secret(key_name)
        return retrieved_key.value
    except Exception as e:
        logging.info("Failed to get the key from Azure Key Vault using DefaultAzureCredential")
        logging.info("The error is caused by: {}".format(e))
    try:
        logging.info("Trying to get the key from Azure Key Vault using DeviceCodeCredential")
...
def get_key_from_local_file(key_name, local_keys_path):
    ...
    if os.path.exists(local_keys_path):
        ...
        if key_name in keys_dict:
            ...
            logging.info("Key not found in local keys file.")
    return key_value, keys_dict


def get_cached_keys_dict(local_keys_path):
